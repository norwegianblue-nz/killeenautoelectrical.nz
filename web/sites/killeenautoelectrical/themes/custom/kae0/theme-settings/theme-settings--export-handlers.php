...
use Drupal\kae0\Theme\OmegaSettingsInfo;
use Drupal\kae0\Export\OmegaExport;

function kae0_theme_generate_validate(&$form, &$form_state) {
  //drupal_set_message(t('Running <strong>kae0_theme_generate_validate()</strong>'));
...
function kae0_theme_generate_submit(&$form, &$form_state) {
  //drupal_set_message(t('Running <strong>kae0_theme_generate_submit()</strong>'));
  $build_info = $form_state->getBuildInfo();
  $themeHandler = \Drupal::service('theme_handler');
  // Get the theme name.
  $theme = $build_info['args'][0];
